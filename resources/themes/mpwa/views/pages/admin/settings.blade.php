<x-layout-dashboard title="{{__('Settings Server')}}">
    ...
    <x-page-header title="{{ __('Setting Server') }}"
        subtitle="{{ __('Configure the Node server, SSL, environment and Meta Cloud API') }}"
        :breadcrumb="[__('Admin'), __('Setting Server')]" />
                ...
                <ul class="nav nav-tabs mb-3" id="myTab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active" id="account-tab" data-bs-toggle="tab" data-bs-target="#server"
                            type="button" role="tab" aria-controls="server"
                            aria-selected="true">{{__('Server')}}</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="meta-tab" data-bs-toggle="tab" data-bs-target="#meta-settings"
    ...
    <div class="card-body">
		...
		@csrf
            ...
            <div class="text-center">
                ...
                {{-- Meta Cloud API Settings Tab --}}
                <div class="tab-pane fade" id="meta-settings" role="tabpanel" aria-labelledby="meta-tab">
                    <div class="card">
                        <div class="card-header">
                            <h5 class="card-title mb-0">{{ __('Meta Cloud API Configuration') }}</h5>
                        </div>
                        <div class="card-body">
                            <div class="alert alert-info">
                                <i class="bi bi-info-circle me-2"></i>
                                {{ __('These settings are used for webhook signature verification. Set the webhook URL in your Meta Developer App to') }}:
                                <code>{{ metaWebhookUrl() }}</code>
                            </div>
                            <form action="{{ route('setEnvAll') }}" method="POST">
                                @csrf
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        <label class="form-label fw-semibold">{{ __('App Secret') }}</label>
                                        <input type="password" name="META_APP_SECRET" class="form-control"
                                            value="{{ env('META_APP_SECRET') }}" placeholder="Your Meta App Secret">
                                        <div class="form-text">{{ __('Used to verify webhook payload signatures (X-Hub-Signature-256).') }}</div>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label fw-semibold">{{ __('Webhook Verify Token') }}</label>
                                        <input type="text" name="META_WEBHOOK_VERIFY_TOKEN" class="form-control"
                                            value="{{ env('META_WEBHOOK_VERIFY_TOKEN') }}" placeholder="Any secret passphrase">
                                        <div class="form-text">{{ __('Must match the Verify Token you enter in Meta Developer App webhook settings.') }}</div>
                                    </div>
                                    <div class="col-12">
                                        <label class="form-label fw-semibold">{{ __('Webhook URL to register in Meta') }}</label>
                                        <div class="input-group">
                                            <input type="text" class="form-control font-monospace" value="{{ metaWebhookUrl() }}" readonly>
                                            <button class="btn btn-outline-secondary" type="button" onclick="navigator.clipboard.writeText('{{ metaWebhookUrl() }}')">
                                ...
                                <div class="mt-3">
                                    <button type="submit" class="btn btn-primary">{{ __('Save Meta Settings') }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
