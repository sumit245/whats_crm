    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header py-2">
                <span class="fw-semibold"><i class="bi bi-funnel-fill me-1"></i>{{ __('Build Audience Segment') }}</span>
            </div>
            ...
            <div class="card-body">
                <form id="segment-form">
                    @csrf
                    <div class="mb-3">
                        <label class="form-label small fw-semibold">{{ __('Segment Name') }}</label>
                        <input type="text" id="seg-name" class="form-control form-control-sm" placeholder="{{ __('e.g. VIP Customers') }}" required>
                    </div>

                    <div class="mb-3">
                        <label class="form-label small fw-semibold">{{ __('Match') }}</label>
                        <select id="seg-operator" class="form-select form-select-sm w-auto">
                    ...
                    <button type="button" id="add-condition" class="btn btn-outline-secondary btn-sm mb-3">
                        <i class="bi bi-plus-lg me-1"></i>{{ __('Add Condition') }}
                    </button>

                    <div class="d-flex align-items-center gap-3">
                        <button type="button" id="preview-btn" class="btn btn-sm btn-outline-primary">
                            <i class="bi bi-eye me-1"></i>{{ __('Preview Count') }}
                        </button>
                        <span id="preview-result" class="text-muted small"></span>
                    </div>
                </form>
            </div>

            <div class="card-footer d-flex justify-content-end gap-2 py-2">
                <a href="{{ route('segments.index') }}" class="btn btn-sm btn-secondary">{{ __('Cancel') }}</a>
                <button type="button" id="save-btn" class="btn btn-sm btn-primary">
                    <i class="bi bi-save me-1"></i>{{ __('Save Segment') }}
                </button>
            </div>
        </div>
    </div>
...
<style>
.condition-row { background: #f8f9fa; border-radius: 6px; padding: 10px 12px; margin-bottom: 8px; }
.condition-row select, .condition-row input { font-size: 0.875rem; }
</style>
...
(function () {
    ...
    function addCondition() {
        ...
        row.id = 'condition-' + id;
        row.innerHTML = `
            <select class="form-select form-select-sm w-auto cond-field" data-id="${id}" onchange="updateOpsAndValue(${id})">
                ${FIELDS.map(f => `<option value="${f.value}">${f.label}</option>`).join('')}
            </select>
            <select class="form-select form-select-sm w-auto cond-op d-none" id="op-${id}">
                ${STRING_OPS.map(o => `<option value="${o.value}">${o.label}</option>`).join('')}
            </select>
            <div class="cond-value-wrap flex-grow-1" id="val-wrap-${id}">
                <input type="text" class="form-control form-control-sm cond-value" id="val-${id}" placeholder="{{ __('value...') }}">
            </div>
            <div class="d-none" id="campaign-wrap-${id}">
                <input type="number" class="form-control form-control-sm cond-campaign-id" id="camp-${id}" placeholder="{{ __('Campaign ID (optional)') }}">
            </div>
    ...
    window.updateOpsAndValue = function (id) {
        ...
        if (field === 'created_at') {
            ...
            valWrap.innerHTML = `<select class="form-select form-select-sm cond-value" id="val-${id}">${DATE_VALUES.map(v => `<option value="${v.value}">${v.label}</option>`).join('')}</select>`;
            ...
            valWrap.innerHTML = `<input type="text" class="form-control form-control-sm cond-value" id="val-${id}" placeholder="{{ __('value...') }}">`;
        }
    };
    ...
    function buildRules() {
        ...
        return { operator: document.getElementById('seg-operator').value, conditions };
    }
    ...
    document.getElementById('preview-btn').addEventListener('click', function () {
        ...
        .then(r => r.json())
        .then(d => { resultEl.textContent = d.error ? d.message : d.count + ' {{ __('contacts match') }}'; });
    });
    ...
    document.getElementById('save-btn').addEventListener('click', function () {
        ...
        .then(r => r.json())
        .then(d => { if (d.error) alert(d.message); else window.location.href = d.redirect; });
    });
