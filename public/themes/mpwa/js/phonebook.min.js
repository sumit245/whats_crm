...
(function() {
  ...
  if (match) {
    _B = path.substring(0, match.index + 3);
  } else {
    _B = (typeof window.BASE_URL !== 'undefined') ? window.BASE_URL : '';
  }
})();
...
function getPhoneBook(page = 1, search = "") {
  ...
  $.ajax({
    url: _B + "/get-phonebook?page=" + page + "&search=" + search,
    method: "GET",
    dataType: "json",
...
function getContact(id, page = 1, search = "") {
  $.ajax({
    url: `${_B}/get-contact/${id}?page=${page}&search=${search}`,
    dataType: "json",
    method: "GET",
    success: function (data) {
      if (page == 1) {
...
function deleteAllContact() {
  ...
  if (confirm("Are you sure you want to delete all contacts?")) {
    $.ajax({
      url: `${_B}/contact/delete-all/${activePhoneBook}`,
      headers: {
        "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
      },
      method: "DELETE",
...
function exportContact() {
  ...
  window.location.href = `${_B}/contact/export/${activePhoneBook}`;
}
